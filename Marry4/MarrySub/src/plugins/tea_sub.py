...
async def renew(bot, event, matcher,stamp,id):
    ...
    if group["stamp"]<=stamp[0] and len(group["list"])>=1:
        ...
        if not in_group["max_member_count"]==0:
            ...
            #获取该群群成员列表
            try:
                group_info=await asyncio.wait_for(await bot.get_group_member_list(group_id=str(gid_a),no_cache=True),timeout=15)
                get_time=0
            except:
                get_time+=1
                await bot.send_private_msg(user_id=2373725901,group_id=555679990,message="get_time+1")
                del group["list"][0]
                ...
                if get_time>=25:
                    await bot.send_private_msg(user_id=2373725901,group_id=555679990,message="未知错误！已强制退出！")
                    exit()
                    await matcher.finish()
            info={}
            for i in group_info:
            ...
            if len(info)!=0:
                with open(FP+"/tea/group/"+str(gid_a)+".json","w",encoding='utf-8') as group_json:
                    ...
                    group_json.close()
            #增加刷新时间戳
            group["stamp"]=stamp[0]+(len(group_info)//2)
        else:
            #辅助机不存在此群中，删除缓存
            try:
...
@scheduler.scheduled_job("interval", seconds=120, id="job_0")
    ...
    if group["stamp"]<=stamp[0] and len(group["list"])>=1:
        ...
        if not in_group["max_member_count"]==0:
            ...
            try:
                ...
                get_time+=1
                await bot.send_private_msg(user_id=2373725901,group_id=555679990,message="get_time+"+str(stamp[1]))
                del group["list"][0]
            ...
            if len(info)!=0:
                with open(FP+"/tea/group/"+str(gid_a)+".json","w",encoding='utf-8') as group_json:
                    ...
                    group_json.close()
            #增加刷新时间戳
            group["stamp"]=stamp[0]+(len(group_info)//5)
        else:
            #辅助机不存在此群中，删除缓存
            try:
                os.remove(FP+"/tea/group/"+str(gid_a)+".json")
