...
async def applicant(bot, event,matcher,stamp,id,iden):
    ...
    elif re.search("关闭pjsk功能",raw_msg) and gid:
        await close_pjsk(bot, event,matcher,stamp,id,iden)
    elif re.search("开启pjsk功能",raw_msg) and gid:
        await open_pjsk(bot, event,matcher,stamp,id,iden)


async def open_pjsk(bot, event,matcher,stamp,id,iden):
    print("DEF:open_pjsk")
    uid=id[0]
    gid=id[1]
    mid=id[2]
    ...
    g3=(await V.selecting(1000,"G"+str(gid),"g3"))[0]
    if g3!=0 and g3!="0":
        if uid==int(g3):
            ...
            msg_1=["text","已开启本群的pjsk功能！"]
            msg_0={"msg":[["reply",str(mid)],msg_1],"type":"G"}
            await W.msg_sent(bot, event,matcher,stamp,id,iden,msg_0)
        else:
            msg_1=["text","( 〞 0 ˄ 0 )错误代码：D-4-1。\n仅领养人可使用此功能！"]
            msg_0={"msg":[["reply",str(mid)],msg_1],"type":"G"}
            await W.msg_sent(bot, event,matcher,stamp,id,iden,msg_0)

async def close_pjsk(bot, event,matcher,stamp,id,iden):
    print("DEF:close_pjsk")
    uid=id[0]
    gid=id[1]
    mid=id[2]
    ggid="G"+str(gid)
...
async def authorization(bot, event,matcher,stamp,id,iden,no):
    ...
    if point==1:
        msg_1=["text","( 〞 0 ˄ 0 )错误代码：D-1-1。\n茉莉主号机不在该群内。"]
        msg_0={"msg":[["reply",str(mid)],msg_1],"type":"G"}
        await W.msg_sent(bot, event,matcher,stamp,id,iden,msg_0)
    elif point==2:
        msg_1=["text","( 〞 0 ˄ 0 )错误代码：D-1-2。\n不存在该群缓存，请在领养群内触发未授权提示后再试。"]
        msg_0={"msg":[["reply",str(mid)],msg_1],"type":"G"}
        await W.msg_sent(bot, event,matcher,stamp,id,iden,msg_0)
    elif point==3:
        msg_1=["text","( 〞 0 ˄ 0 )错误代码：D-1-3。\n本群已授权且已存在领养人。若想更改领养人，请联系茉莉的主人音奈更改。_n_不过更新了一下pjsk的授权~"]
        msg_0={"msg":[["reply",str(mid)],msg_1],"type":"G"}
        await W.msg_sent(bot, event,matcher,stamp,id,iden,msg_0)
        #haruki
        haruki_url="http://127.0.0.1:2525/haruki_client/controller/add_whitelist"
